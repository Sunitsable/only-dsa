...
class Stack {
    ...
    void push( int element) {
        if(size - top > 1) {   //example : if size is 5 and the the top is at index 4 then 5-4=1 so the stack is full so we cant put more elements in it
    ...
    void pop() {
        if(top >=0 ) {  //there is only one condition to pop to not work that is if there are no elements then we cant pop any element so top index should always be more than -1 
